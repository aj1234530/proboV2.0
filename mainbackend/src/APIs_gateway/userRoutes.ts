...
router.post("/register", async (req: Request, res: Response) => {
  try {
    ...
    const response = await redisClient.brpop(
      `responses:register_user_${uniqueId}`,
      30
    ); //wait unitl a time out //looking for a specific response
    if (response) {
      console.log(response);
      res
        .status(200)
        .send(
          `your request for user registration has been  fullfilled with orderId: ${uniqueId}`
        );
    } else {
...
router.post("/orderbook", (req: Request, res: Response) => {
  //get orderbook
});
